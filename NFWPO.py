...
class FrankWolfeAgent:
    ...
    def fw_update(self):
        ...
        for i in range(self.batch_size):
            ...
            action = actions[i]
            if self.constraint_satisfied(action, state):
                constrained_actions.append(action)
            else:
                constrained_action = self.projection(action, state)
                constrained_actions.append(constrained_action)
        constrained_actions = torch.FloatTensor(constrained_actions).to(device)

        # Compute gradients of critic w.r.t actions
        self.critic.eval()
        q_values = self.critic(state_batch, constrained_actions)
        action_grads = torch.autograd.grad(q_values.mean(), constrained_actions, retain_graph=True)[0]
        self.critic.train()

        # Frank-Wolfe update
        action_table = []
        for i in range(self.batch_size):
            grad = action_grads[i].cpu().detach().numpy()
            state = state_batch[i].cpu().numpy()
            ...
            lr = 0.01
            a_old = constrained_actions[i].cpu().numpy()
            a_new = a_old + lr * (s - a_old)
        ...
        actor_loss = nn.MSELoss()(self.actor(state_batch), action_table)
        actor_loss.backward()
    ...
    def solve_linear_subproblem(self, grad, state):
        # Solve the linear subproblem in the Frank-Wolfe algorithm
        try:
            ...
            return np.zeros(self.action_dim)  # Return zero action if optimization fails
