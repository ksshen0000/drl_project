...
class ReplayBuffer:
    ...
    def push(self, state, action, reward, next_state, done):
        """Store a transition and compute n-step return."""
        transition = (state, action, reward, next_state, done)
        self.n_step_buffer.append(transition)
        
        if len(self.n_step_buffer) < self.n_step:
            return
        # Compute n-step return and next state
        reward_n, next_state_n, done_n = self.compute_n_step_return()
        state_n, action_n = self.n_step_buffer[0][:2]
        # Store n-step transition
        self._store_transition(state_n, action_n, reward_n, next_state_n, done_n)
        self.n_step_buffer.pop(0)
        if done:
            self.n_step_buffer.clear()
    
    def compute_n_step_return(self):
        """Compute n-step return for current buffer."""
    ...
    def _store_transition(self, state, action, reward, next_state, done):
        """Internal method to store a transition."""
...
# SAC Agent
class SAC:
    def __init__(self, state_dim, action_dim, action_bound, n_step=3):
        ...
        # Replay Buffer
        self.memory = ReplayBuffer(MEMORY_CAPACITY, n_step=self.n_step, gamma=self.gamma)
        
        # Counter for delayed policy updates
        self.total_it = 0
    
    def compute_kl(self, old_mean, old_log_std, new_mean, new_log_std):
        """Compute KL divergence between old and new policy."""
        old_std = old_log_std.exp()
        new_std = new_log_std.exp()
        
        kl = (new_log_std - old_log_std) + (old_std.pow(2) + (old_mean - new_mean).pow(2)) / (2.0 * new_std.pow(2)) - 0.5
    ...
    def update_parameters(self):
        ...
        with torch.no_grad():
            ...
            # Use n-step returns
            target_q = reward + (1 - done) * (self.gamma ** self.n_step) * target_q
        ...
        if self.total_it % POLICY_FREQ == 0:
            ...
            new_mean, new_log_std = self.actor.forward(state)
            new_std = new_log_std.exp()
            new_normal = Normal(new_mean, new_std)
            x_t = new_normal.rsample()
            y_t = torch.tanh(x_t)
            new_action = y_t * self.action_bound
            # Compute log probability
            log_prob = new_normal.log_prob(x_t)
            log_prob -= torch.log(self.action_bound * (1 - y_t.pow(2)) + 1e-6)
            log_prob = log_prob.sum(1, keepdim=True)
            
            # Compute old policy
            with torch.no_grad():
                old_mean, old_log_std = self.actor.forward(state)
            
            # Compute KL divergence
            kl_div = self.compute_kl(old_mean, old_log_std, new_mean, new_log_std)
            kl_mean = kl_div.mean()
            
            # Set KL divergence limit
            kl_limit = 0.01  # You can adjust this value
            
            if kl_mean > kl_limit:
                # Skip this update if KL divergence is too high
                return
...
if __name__ == "__main__":
    ...
    model_dir = os.path.join('models', args.env_name+'_sac_trpo')
