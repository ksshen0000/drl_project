...
# Ornstein-Uhlenbeck Noise for exploration (optional but recommended)
class OUNoise:
...
class TD3:
    ...
    def update(self):
        ...
        next_action = (self.actor_target(next_state) + noise).clamp(-self.actor.action_bound, self.actor.action_bound)
        ...
        # Compute critic loss
        critic_loss = self.loss_fn(current_q1, target_q.detach()) + self.loss_fn(current_q2, target_q.detach())
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='TD3 for OpenAI Gym Environments')
    ...
    if args.load:
        ...
        checkpoint_path = os.path.join(model_dir, 'td3_checkpoint.pth')
        buffer_path = os.path.join(model_dir, 'replay_buffer.npy')
    ...
    for episode in range(start_episode, MAX_EPISODES):
        ...
        if (episode + 1) % SAVE_INTERVAL == 0:
            checkpoint_path = os.path.join(model_dir, f'td3_checkpoint_ep{episode + 1}.pth')
            buffer_path = os.path.join(model_dir, f'replay_buffer_ep{episode + 1}.npy')
