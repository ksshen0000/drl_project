...
class PPO:
    ...
    def update(self):
        ...
        # Compute discounted returns using Monte Carlo method (without bootstrapping)
        returns = []
        Gt = 0
        for reward, done in zip(reversed(reward_arr), reversed(done_arr)):
            ...
            returns.insert(0, Gt)
            if done:
                Gt = 0  # Reset cumulative return if episode ends

        returns = torch.FloatTensor(returns).to(self.device)

        # Compute advantages: Advantage = Returns - Baseline (using Monte Carlo estimates)
        with torch.no_grad():
            state_values = self.critic(state_arr).squeeze()
        ...
        for _ in range(self.epochs):
            for index in range(0, len(self.memory), self.batch_size):
                ...
                advantages_batch = advantages[batch_slice]

                # Recompute action distributions and state values
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Proximal Policy Optimization (PPO) with Monte Carlo for OpenAI Gym Environments')
    ...
    model_dir = os.path.join('models', args.env_name + '_ppo_mc')
