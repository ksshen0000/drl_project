...
class DDPG(AbstractSolver):
    ...
    def compute_target_values(self, next_states, rewards, dones):
        ...
        ################################
        next_greedy_actions = self.target_actor_critic.pi(next_states)
        next_q_values = self.target_actor_critic.q(next_states, next_greedy_actions)
        target_q = rewards + (1 - dones) * self.options.gamma * next_q_values
        return target_q
    ...
    def train_episode(self):
        ...
        for _ in range(self.options.steps):
            ...
            #   YOUR IMPLEMENTATION HERE   #
            ################################
            curr_action = self.select_action(state)
            next_state, reward, done, _ = self.step(curr_action)
            self.memorize(state, curr_action, reward, next_state, done)
            self.replay()
